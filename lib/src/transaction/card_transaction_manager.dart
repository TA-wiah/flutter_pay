...
import 'package:paystack_pay/src/api/request/card_request_body.dart';
import 'package:paystack_pay/src/api/request/validate_request_body.dart';
import 'package:paystack_pay/src/api/service/contracts/cards_service_contract.dart';
import 'package:paystack_pay/src/common/exceptions.dart';
...
import 'package:paystack_pay/src/transaction/base_transaction_manager.dart';
...
class CardTransactionManager extends BaseTransactionManager {
  ...
  @override
  postInitiate() async {
    chargeRequestBody = await CardRequestBody.getChargeRequestBody(
      publicKey,
      charge,
    );
  ...
  Future<CheckoutResponse> _reQueryChargeOnServer() {
    Future<TransactionApiResponse> future = service.reQueryTransaction(
      transaction.id,
    );
    return handleServerResponse(future);
  }
  ...
  Future<CheckoutResponse> sendChargeOnServer() {
    Future<TransactionApiResponse> future = service.chargeCard(
      chargeRequestBody.paramsMap(),
    );
    return handleServerResponse(future);
  }
